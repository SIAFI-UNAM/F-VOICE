...
def main():
    ...
    if n_gpus > 1:
        ...
        run(0, 1, hps)  # Para una sola GPU, ejecuta sin multiprocessing
...
def run(rank, n_gpus, hps):
    ...
    if torch.cuda.device_count() > 1:  # Si hay más de 1 GPU, usa DDP

        net_g = DDP(net_g, device_ids=[rank], find_unused_parameters=True)
        net_d = DDP(net_d, device_ids=[rank], find_unused_parameters=True)
    else:
        net_g = torch.nn.DataParallel(net_g) if torch.cuda.device_count() == 1 else net_g.to(torch.device("cuda"))
        net_d = net_d.to(torch.device("cuda"))  # Asegurar que el modelo está en la GPU

    # Ajustar net_dur_disc si existe
    if net_dur_disc is not None:
        net_dur_disc = (
            DDP(net_dur_disc, device_ids=[rank], find_unused_parameters=True)
            if torch.cuda.device_count() > 1
            else torch.nn.DataParallel(net_dur_disc) if torch.cuda.device_count() == 1 else net_dur_disc.to(torch.device("cuda"))
        )
