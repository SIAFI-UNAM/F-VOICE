...
def mel_creator(input_path,output_path,window='hann',max_wav_value=32768.0,sampling_rate=22050,filter_length=1024,hop_length=256,win_length=1024,n_mel_channels=80,mel_fmin=0.0,mel_fmax=8000.0):  
#directory = 'E:\Github\F-voice\F-VOICE\Mel_creator\wavs_mel'
    avs = glob.glob(input_path +'/*.wav')
    for i in tqdm.tqdm(range(len(avs)), desc ="Mels state"): 
        time.sleep(.1)
        ...
        file_name = avs[i].replace(input_path,"").replace(".wav","")
        file_path = output_path + file_name 
        
        np.save(file_path, mel_output)
    print('wavs_mel created')